...
const CourseCatalog: React.FC = () => {
  ...
  const categories = [
    ...
    { id: 'programing', name: t.catalog.categories.programing },
    { id: 'ciybersecyrity', name: t.catalog.categories.ciybersecyrity },
  ];
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
              ...
              <div className="py-1">
                {categories.map((category) => (
                  <button
                    ...
                    className={`${
                      selectedCategory === category.id ? 'bg-teal-50 text-teal-700' : 'text-gray-700'
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredCourses.map((course) => (
          <div key={course.id} className="bg-white rounded-lg shadow-md overflow-hidden transition-transform duration-300 hover:transform hover:scale-105">
            <div className="relative pb-1/2">
              <img src={course.image}  className="absolute h-full w-full object-cover" />
            </div>
            <div className="p-4">
              <div className="flex items-center justify-between">
                <span className="inline-block bg-teal-100 text-teal-800 text-xs px-2 py-1 rounded-full">
                  {t.catalog.categories[course.category as keyof typeof t.catalog.categories]}
                </span>
                <span className="text-sm text-gray-500">{course.duration}</span>
              </div>
              <h3 className="mt-2 text-lg font-medium text-gray-900">{course.title[language]}</h3>
              ...
              <div className="mt-3 flex items-center">
                ...
                <span className="ml-2 text-sm text-gray-500">({course.reviewCount})</span>
              </div>
              <div className="mt-4">
                <a
                  href="#"
